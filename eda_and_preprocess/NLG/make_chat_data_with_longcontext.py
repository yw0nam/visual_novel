...
for i in tqdm(range(len(main_chara_data))):
    ...
    out = data.loc[index-context_size:index].apply(lambda x:
        ...
        f"{x['name']}: {x['text_remove_yomigana']}"
    ,axis=1).to_list()
    out_ls.append({
        'chat_template': [
            {
            ...
            {
                'role': 'user', 
                "content": f"Here is Context. You have to response properly given context.\n\n"+"\n".join(out[:-1])
            },
            {
                'role': 'assistant', 
                "content": out[-1]
...
# %%
jdump(train.to_list(), '/data/research_data/dataset/visual_novel/llm/ver_1.2/train.json')
jdump(val.to_list(), '/data/research_data/dataset/visual_novel/llm/ver_1.2/val.json')
# %%
# %%
...
# %%
data.groupby(by='scene_name').size()
# %%
data.query("scene_name == 'chapter　5-1-02'")
# %%
